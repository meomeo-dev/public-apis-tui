...
test('provider config creates CLI options and query without secrets', () => {
  const env = {
    MARKETAUX_LIMIT: '2',
    MARKETAUX_SEARCH: 'markets',
    MARKETAUX_SYMBOLS: 'TSLA,MSFT',
    MARKETAUX_COUNTRIES: 'us',
  ...
  assert.equal(options.includes('--limit'), true)
  assert.equal(options.includes('--search'), true)
  assert.equal(query.search, 'markets')
  assert.equal(query.symbols, 'TSLA,MSFT')
})
...
test('summarizer preserves all large-batch candidates before Claude prompt', () => {
  ...
  assert.doesNotMatch(source, /MARKETAUX_LIMIT.*30/u)
})
