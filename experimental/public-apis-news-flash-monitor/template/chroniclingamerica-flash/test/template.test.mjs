...
test('provider config creates CLI options and query without secrets', () => {
  const env = {
    CHRONICLINGAMERICA_QUERY: 'lincoln',
    CHRONICLINGAMERICA_COUNT: '2',
    CHRONICLINGAMERICA_PAGE: '3',
    CHRONICLINGAMERICA_DATES: '1860/1865',
    CHRONICLINGAMERICA_STATE: 'ny',
  }
  ...
  assert.equal(options.includes('--state'), false)
})
...
test('summarizer preserves all large-batch candidates before Claude prompt', () => {
  ...
  assert.doesNotMatch(source, /CHRONICLINGAMERICA_COUNT.*30/u)
})
