...
test('provider config creates CLI options and query without secrets', () => {
  const env = {
    GUARDIAN_PAGE_SIZE: '2',
    GUARDIAN_QUERY: 'markets',
    GUARDIAN_SECTION: 'technology',
    GUARDIAN_TAG: 'technology/apple',
  ...
  assert.equal(options.includes('--page-size'), true)
  assert.equal(options.includes('--section'), true)
  assert.equal(query.query, 'markets')
  assert.equal(query.section, 'technology')
})
...
test('summarizer preserves all large-batch candidates before Claude prompt', () => {
  ...
  assert.doesNotMatch(source, /GUARDIAN_PAGE_SIZE.*30/u)
})
