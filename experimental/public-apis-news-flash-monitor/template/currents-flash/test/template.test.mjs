...
test('provider config creates CLI options and query without secrets', () => {
  const env = {
    CURRENTS_PAGE_SIZE: '2',
    CURRENTS_KEYWORDS: 'markets',
    CURRENTS_LANGUAGE: 'en',
    CURRENTS_COUNTRY: 'us',
    CURRENTS_CATEGORY: 'technology',
    CURRENTS_PAGE: '3',
  ...
  assert.equal(JSON.stringify(query).includes('test-key'), false)
})
...
test('summarizer preserves all large-batch candidates before Claude prompt', () => {
  ...
  assert.doesNotMatch(source, /CURRENTS_PAGE_SIZE.*30/u)
})
