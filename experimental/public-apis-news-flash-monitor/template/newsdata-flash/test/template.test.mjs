...
test('provider config creates CLI options and query without secrets', () => {
  const env = {
    NEWSDATAIO_API_KEY: 'test-key',
    NEWSDATA_CATEGORY: 'business',
    NEWSDATA_COUNTRY: 'us',
    NEWSDATA_DEDUPE: 'true',
    NEWSDATA_DOMAIN: 'example',
    NEWSDATA_LANGUAGE: 'en',
    NEWSDATA_PAGE: 'next-token',
    NEWSDATA_QUERY: 'markets',
    NEWSDATA_SEARCH_IN: 'title',
    NEWSDATA_SIZE: '2',
    NEWSDATA_SORT: 'relevancy',
  }
...
test('summarizer preserves all large-batch candidates before Claude prompt', () => {
  ...
  assert.doesNotMatch(source, /NEWSDATA_SIZE.*30/u)
})
