...
test('provider config creates CLI options and query without secrets', () => {
  const env = {
    THENEWS_SEARCH: 'markets',
    THENEWS_LANGUAGE: 'en',
    THENEWS_LOCALE: 'us',
    THENEWS_CATEGORIES: 'business,tech',
    THENEWS_DOMAINS: 'example.com',
    THENEWS_PUBLISHED_AFTER: '2026-05-01',
    THENEWS_PUBLISHED_BEFORE: '2026-05-10',
    THENEWS_PUBLISHED_ON: '2026-05-09',
    THENEWS_SORT: 'published_at',
    THENEWS_LIMIT: '2',
    THENEWS_PAGE: '3',
  }
...
test('summarizer sanitizes Claude error diagnostics', () => {
  ...
  assert.doesNotMatch(source, /\bNEWSAPI_API_KEY\b/)
...
test('summarizer preserves all large-batch candidates before Claude prompt', () => {
  ...
  assert.doesNotMatch(source, /THENEWS_LIMIT.*30/u)
})
