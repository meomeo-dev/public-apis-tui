...
const defaultPackageId = '6d3d7654-4992-4203-92e8-81bfd6fd258b'
...
test('Open Government UK live e2e covers search, dataset, and offline replay', { skip: liveEnabled ? false : 'set PUBLIC_APIS_LIVE_E2E=1 to call live public APIs' }, async () => {
  await withPublicApisHome(async publicApisHome => {
    const env = { ...process.env, PUBLIC_APIS_HOME_DIR: publicApisHome, NO_COLOR: '1' }
    const search = await runJson<SearchResult>(['apis', 'run', 'opengovernmentuk.search', '--online', '--persist', '--format', 'json', '--', '--query', 'business', '--limit', '1000'], env)
    assert.equal(search.kind, 'opengovernmentuk.search')
    assert.equal(search.api.provider, 'opengovernmentuk')
    ...
    const searchOffline = await runJson<SearchResult>(['apis', 'run', 'opengovernmentuk.search', '--offline', '--format', 'json', '--', '--query', 'business', '--limit', '1000'], env)
    assert.equal(searchOffline.storage.mode, 'offline')
    assert.deepEqual(searchOffline.datasets, search.datasets)

    const dataset = await runJson<DatasetResult>(['apis', 'run', 'opengovernmentuk.dataset', '--online', '--persist', '--format', 'json', '--', '--package-id', defaultPackageId], env)
    ...
    const datasetOffline = await runJson<DatasetResult>(['apis', 'run', 'opengovernmentuk.dataset', '--offline', '--format', 'json', '--', '--package-id', defaultPackageId], env)
    assert.equal(datasetOffline.storage.mode, 'offline')
    assert.deepEqual(datasetOffline.dataset, dataset.dataset)

    const text = await runCli(['apis', 'run', 'opengovernmentuk.dataset', '--offline', '--format', 'text', '--', '--package-id', defaultPackageId], env)
    assert.match(text.stdout, /Open Government UK Dataset Detail/)
...
async function withPublicApisHome(run: (publicApisHome: string) => Promise<void>): Promise<void> {
  const publicApisHome = await mkdtemp(join(tmpdir(), 'public-apis-live-opengovernmentuk-'))
  try {
