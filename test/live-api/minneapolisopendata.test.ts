...
test('Open Data Minneapolis live e2e covers datasets and offline replay', { skip: liveEnabled ? false : 'set PUBLIC_APIS_LIVE_E2E=1 to call live public APIs' }, async () => {
  await withPublicApisHome(async publicApisHome => {
    const env = { ...process.env, PUBLIC_APIS_HOME_DIR: publicApisHome, NO_COLOR: '1' }
    const datasets = await runJson<DatasetsResult>(['apis', 'run', 'minneapolisopendata.datasets', '--online', '--persist', '--format', 'json', '--', '--query', 'transportation', '--limit', '100'], env)
    assert.equal(datasets.kind, 'minneapolisopendata.datasets')
    assert.equal(datasets.api.provider, 'minneapolisopendata')
    ...
    const datasetsOffline = await runJson<DatasetsResult>(['apis', 'run', 'minneapolisopendata.datasets', '--offline', '--format', 'json', '--', '--query', 'transportation', '--limit', '100'], env)
    assert.equal(datasetsOffline.storage.mode, 'offline')
    assert.deepEqual(datasetsOffline.datasets, datasets.datasets)

    const text = await runCli(['apis', 'run', 'minneapolisopendata.datasets', '--offline', '--format', 'text', '--', '--query', 'transportation', '--limit', '100'], env)
    assert.match(text.stdout, /Open Data Minneapolis Datasets/)
...
async function withPublicApisHome(run: (publicApisHome: string) => Promise<void>): Promise<void> {
  const publicApisHome = await mkdtemp(join(tmpdir(), 'public-apis-live-minneapolisopendata-'))
  try {
