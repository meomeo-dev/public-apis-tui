...
const defaultPackageId = '2d90548d-50ef-4802-91f8-c59c5cf68251'
...
test('Open Government Canada live e2e covers search, dataset, and offline replay', { skip: liveEnabled ? false : 'set PUBLIC_APIS_LIVE_E2E=1 to call live public APIs' }, async () => {
  await withPublicApisHome(async publicApisHome => {
    const env = { ...process.env, PUBLIC_APIS_HOME_DIR: publicApisHome, NO_COLOR: '1' }
    const search = await runJson<SearchResult>(['apis', 'run', 'opengovernmentcanada.search', '--online', '--persist', '--format', 'json', '--', '--query', 'business', '--limit', '1000'], env)
    assert.equal(search.kind, 'opengovernmentcanada.search')
    assert.equal(search.api.provider, 'opengovernmentcanada')
    ...
    const searchOffline = await runJson<SearchResult>(['apis', 'run', 'opengovernmentcanada.search', '--offline', '--format', 'json', '--', '--query', 'business', '--limit', '1000'], env)
    assert.equal(searchOffline.storage.mode, 'offline')
    assert.deepEqual(searchOffline.datasets, search.datasets)

    const dataset = await runJson<DatasetResult>(['apis', 'run', 'opengovernmentcanada.dataset', '--online', '--persist', '--format', 'json', '--', '--package-id', defaultPackageId], env)
    ...
    const datasetOffline = await runJson<DatasetResult>(['apis', 'run', 'opengovernmentcanada.dataset', '--offline', '--format', 'json', '--', '--package-id', defaultPackageId], env)
    assert.equal(datasetOffline.storage.mode, 'offline')
    assert.deepEqual(datasetOffline.dataset, dataset.dataset)

    const text = await runCli(['apis', 'run', 'opengovernmentcanada.dataset', '--offline', '--format', 'text', '--', '--package-id', defaultPackageId], env)
    assert.match(text.stdout, /Open Government Canada Dataset Detail/)
...
async function withPublicApisHome(run: (publicApisHome: string) => Promise<void>): Promise<void> {
  const publicApisHome = await mkdtemp(join(tmpdir(), 'public-apis-live-opengovernmentcanada-'))
  try {
