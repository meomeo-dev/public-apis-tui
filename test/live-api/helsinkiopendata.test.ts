...
const defaultPackageId = '0ba02e5d-9f06-496a-8354-bb15beec5629'
...
test('Helsinki Region Infoshare live e2e covers search, dataset, and offline replay', { skip: liveEnabled ? false : 'set PUBLIC_APIS_LIVE_E2E=1 to call live public APIs' }, async () => {
  await withPublicApisHome(async publicApisHome => {
    const env = { ...process.env, PUBLIC_APIS_HOME_DIR: publicApisHome, NO_COLOR: '1' }
    const search = await runJson<SearchResult>(['apis', 'run', 'helsinkiopendata.search', '--online', '--persist', '--format', 'json', '--', '--query', 'transport', '--limit', '100'], env)
    assert.equal(search.kind, 'helsinkiopendata.search')
    assert.equal(search.api.provider, 'helsinkiopendata')
    ...
    const searchOffline = await runJson<SearchResult>(['apis', 'run', 'helsinkiopendata.search', '--offline', '--format', 'json', '--', '--query', 'transport', '--limit', '100'], env)
    assert.equal(searchOffline.storage.mode, 'offline')
    assert.deepEqual(searchOffline.datasets, search.datasets)

    const dataset = await runJson<DatasetResult>(['apis', 'run', 'helsinkiopendata.dataset', '--online', '--persist', '--format', 'json', '--', '--package-id', defaultPackageId], env)
    ...
    const datasetOffline = await runJson<DatasetResult>(['apis', 'run', 'helsinkiopendata.dataset', '--offline', '--format', 'json', '--', '--package-id', defaultPackageId], env)
    assert.equal(datasetOffline.storage.mode, 'offline')
    assert.deepEqual(datasetOffline.dataset, dataset.dataset)

    const text = await runCli(['apis', 'run', 'helsinkiopendata.dataset', '--offline', '--format', 'text', '--', '--package-id', defaultPackageId], env)
    assert.match(text.stdout, /Helsinki Region Infoshare Dataset Detail/)
...
async function withPublicApisHome(run: (publicApisHome: string) => Promise<void>): Promise<void> {
  const publicApisHome = await mkdtemp(join(tmpdir(), 'public-apis-live-helsinkiopendata-'))
  try {
